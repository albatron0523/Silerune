...
const OceanEyesShowcase: React.FC<{ isBackgroundPlaying: boolean, setIsBackgroundPlaying: (val: boolean) => void, isEditMode?: boolean, onEdit?: () => void }> = ({ isBackgroundPlaying, setIsBackgroundPlaying, isEditMode, onEdit }) => {
    ...
    <div className={`master-container-pv relative ${isReady ? 'playing-pv' : ''}`} id="main-pv">
      ...
      <div className="ui-side-pv">
        <div className="header-area-pv">
          <h1 className="title-pv">Ocean eyes</h1>
  ...
  return (
    <div className={`observation-card relative ${isCorrupted ? 'corrupted' : ''}`} style={{ 
      ...
      <div className="card-controls">
        ...
        <button className="toggle-btn" onClick={() => setShowSelector(true)}>
          切換檔案
        </button>
      </div>
      ...
      {showSelector && (
        <div className="absolute inset-0 bg-white/95 backdrop-blur-sm z-[150] p-6 flex flex-col justify-start overflow-y-auto rounded-lg">
          <div className="flex justify-between items-center border-b border-gray-100 pb-2 mb-4">
            <span className="text-sm font-bold text-gray-700">選擇觀察檔案</span>
            <button 
          ...
          <div className="grid grid-cols-2 gap-2">
            {observationState.map((c) => (
              <button
                ...
                style={{
                  borderColor: charId === c.id ? c.colors.P : '#e5e7eb',
                  backgroundColor: charId === c.id ? `${c.colors.P}0d` : '#ffffff',
                  color: charId === c.id ? c.colors.P : '#374151',
                }}
              >
...
export default function App() {
  ...
  const handleSaveArtMetadata = () => {
    ...
    setGalleryItemsState(tempGalleryItemsState);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="w-full">
          ...
          <div className="sub-page active">
            <div className="top-content">
              ...
              <div className="intro-section relative">
                ...
                <div className="line line-1">{prefaceConfig.line1}</div>
                <div className="line line-2">{prefaceConfig.line2}</div>
              </div>

              <div className="timer-section relative">
                {isEditMode && (
                  <button 
              ...
              <section className="credits-container">
                ...
                <div className="credits-buttons">
                  ...
                  <a href="https://youtu.be/viimfQi_pUw?si=0VPGDEpk8XviBodF" target="_blank" rel="noopener noreferrer" className="credit-btn">
                    OCEAN EYES
                  </a>
                  <button 
        ...
        {activeTab === 'art' && (
          <div className="sub-page active">
            <OceanEyesShowcase isBackgroundPlaying={isPlaying} setIsBackgroundPlaying={setIsPlaying} isEditMode={isEditMode} onEdit={() => setActiveEditSection('文章影片播放器')} />
            <div className={`gallery-outer relative ${selectedArt ? 'detail-mode' : ''}`}>
              <div className="gallery-title flex items-center justify-center gap-2">
              ...
              <div className="scroll-container">
                {galleryItemsState.map((col: any, idx) => (
                  <div key={idx} className={`art-col col-${col.type}`}>
                    {col.type === 'portrait' && (
                      <div className={`art-frame relative ${selectedArt === col.url ? 'selected' : ''}`} onClick={() => setSelectedArt(selectedArt === col.url ? null : col.url)}>
                        <img src={col.url} alt="Art" referrerPolicy="no-referrer" />
                        {isEditMode && (
                          <button 
                            onClick={(e) => { e.stopPropagation(); setActiveEditSection('畫廊'); }}
                            className="absolute top-2 right-2 w-6 h-6 rounded-full bg-[#bdbade] hover:bg-[#8a7f9c] flex items-center justify-center text-white text-[10px] font-bold shadow-sm z-[100] transition-all cursor-pointer hover:scale-105"
                          >
                            ✎
                          </button>
                        )}
                        {selectedArt === col.url && (
                          <div className="art-info-panel">
                            <div className="info-item"><span>作品名：</span>{getArtInfo(col.url).title}</div>
                            <div className="info-item"><span>創作日期：</span>{getArtInfo(col.url).date}</div>
                            <div className="info-item"><span>繪師：</span>{getArtInfo(col.url).artist}</div>
                          </div>
                        )}
                      </div>
                    )}
                    {col.type === 'stack' && col.urls.map((url: string, uidx: number) => (
                      <div key={uidx} className={`art-frame relative ${selectedArt === url ? 'selected' : ''}`} onClick={() => setSelectedArt(selectedArt === url ? null : url)}>
                      ...
                      <>
                        <div className="grid-row">
                          {col.topRow.map((url: string, tidx: number) => (
                            <div key={tidx} className={`art-frame relative ${selectedArt === url ? 'selected' : ''}`} onClick={() => setSelectedArt(selectedArt === url ? null : url)}>
                              <img src={url} alt="Art" referrerPolicy="no-referrer" />
                              {isEditMode && (
                                <button 
                                  onClick={(e) => { e.stopPropagation(); setActiveEditSection('畫廊'); }}
                        ...
                        <div className="grid-row">
                          {col.bottomRow.map((url: string, bidx: number) => (
                            <div key={bidx} className={`art-frame relative ${selectedArt === url ? 'selected' : ''}`} onClick={() => setSelectedArt(selectedArt === url ? null : url)}>
                              <img src={url} alt="Art" referrerPolicy="no-referrer" />
                              {isEditMode && (
                                <button 
                                  onClick={(e) => { e.stopPropagation(); setActiveEditSection('畫廊'); }}
                    ...
                    {col.type === 'complex' && (
                      <>
                        <div className={`art-frame relative top-frame ${selectedArt === col.top ? 'selected' : ''}`} onClick={() => setSelectedArt(selectedArt === col.top ? null : col.top)}>
                          ...
                          {selectedArt === col.top && (
                            <div className="art-info-panel">
                              <div className="info-item"><span>作品名：</span>{getArtInfo(col.top).title}</div>
                              <div className="info-item"><span>創作日期：</span>{getArtInfo(col.top).date}</div>
                              <div className="info-item"><span>繪師：</span>{getArtInfo(col.top).artist}</div>
                            </div>
                          )}
                        </div>
                ...
                {activeEditSection === '世界觀' && tempWikiData ? (
                  ...
                  <div className="flex-grow flex flex-col justify-start items-stretch text-left overflow-hidden">
                    <div className="space-y-4 overflow-y-auto pr-2 flex-grow max-h-[55vh] flex flex-col">
                      ...
                      {/* Upload Button */}
                      <div className="flex flex-col gap-1 flex-shrink-0">
                        <label className="text-xs font-bold text-gray-500">上傳：</label>
                        <label className="w-full flex flex-col items-center justify-center border-2 border-dashed border-gray-200 hover:border-[#8a7f9c] rounded-lg p-4 cursor-pointer transition-colors bg-gray-50 hover:bg-gray-50/50">
                          <span className="text-xs font-bold text-[#8a7f9c] mb-1">＋ 選擇圖片並上傳</span>
                          <span className="text-[10px] text-gray-400">支援 PNG, JPG, GIF 等格式</span>
                          <input 
                            type="file" 
                            accept="image/*" 
                            className="hidden" 
                              ...
                              if (file) {
                                ...
                                reader.onload = (event) => {
                                  if (event.target?.result) {
                                    ...
                                    const uniqueName = `uploaded_${Date.now()}_${file.name}`;
                                    
                                    const updatedMeta = { ...tempArtMetadataState };
                                    updatedMeta[uniqueName] = {
                                      title: file.name.split('.').pop() ? file.name.substring(0, file.name.lastIndexOf('.')) : file.name,
                                      date: new Date().toLocaleDateString('zh-TW', { year: 'numeric', month: 'long', day: 'numeric' }),
                                      artist: '您'
                                    };
                                    setTempArtMetadataState(updatedMeta);

                                    const updatedItems = [...tempGalleryItemsState, { type: 'portrait', url: dataUrl }];
                      ...
                      <div className="flex-grow flex flex-col">
                        ...
                        <div className="space-y-4 pr-1">
                          {tempGalleryItemsState.map((item: any, idx: number) => {
                            const urls: string[] = [];
                            if (item.type === 'portrait' && item.url) urls.push(item.url);
                            else if (item.type === 'stack' && item.urls) urls.push(...item.urls);
                            else if (item.type === 'grid-2x2') {
                              if (item.topRow) urls.push(...item.topRow);
                              if (item.bottomRow) urls.push(...item.bottomRow);
                            } else if (item.type === 'complex') {
                              if (item.top) urls.push(item.top);
                              if (item.bottom) urls.push(...item.bottom);
                            }

                            return (
                              <div key={idx} className="border border-gray-100 rounded-lg p-3 bg-gray-50/50 flex flex-col gap-2">
                                <div className="flex justify-between items-center border-b border-gray-100 pb-1.5">
                                  <span className="text-xs text-gray-400 font-bold">作品群組 {idx + 1} ({item.type})</span>
                                  <button
                                    onClick={() => {
                                ...
                                <div className="flex gap-3">
                                  ...
                                  <div className="flex-grow space-y-3">
                                    {urls.map((url, uidx) => {
                                      const filename = decodeURIComponent(url.split('/').pop() || '');
                                      const metadata = tempArtMetadataState[filename] || { title: '新作品', date: '', artist: '' };
                                      return (
                                        <div key={uidx} className="border-l-2 border-[#8a7f9c]/30 pl-2 py-1 space-y-1">
                      ...
                      <div className="flex flex-col gap-1 flex-shrink-0">
                        ...
                        <div className="flex gap-2 items-center">
                          <input 
                            ...
                            readOnly
                            className="flex-1 px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 text-xs font-mono truncate"
                            placeholder="請選擇上傳音檔"
                          />
                          <label className="px-4 py-2 bg-[#8a7f9c] hover:bg-[#746db5] text-white rounded-lg text-xs font-bold cursor-pointer transition-colors flex-shrink-0">
                            上傳
                            <input 
                              type="file" 
                              accept="audio/*" 
                      ...
                      return (
                        <div className="flex-grow flex flex-col justify-start items-stretch text-left overflow-hidden h-[55vh]">
                          <div className="space-y-4 overflow-y-auto pr-2 flex-grow">
                            ...
                            <div className="flex flex-col gap-1">
                              <label className="text-xs font-bold text-gray-500">圖片路徑 / 上傳新圖片：</label>
                              <div className="flex gap-2">
                                <input 
                                  type="text"
                                  ...
                                  onChange={(e) => {
                                    const updated = { ...tempCharIntroState };
                                    updated[key].imageUrl = e.target.value;
                                    setTempCharIntroState(updated);
                                    setIsDirty(true);
                                  }}
                                  className="flex-grow px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8a7f9c] text-xs text-gray-700 font-mono"
                                  placeholder="https://..."
                                />
                                <label className="flex-shrink-0 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-200 rounded-lg text-xs font-semibold cursor-pointer transition-colors flex items-center justify-center">
                                  上傳
                                  <input 
                                    type="file"
                                    accept="image/*"
                                    ...
                                    onChange={(e) => {
                                      ...
                                      if (file) {
                                        ...
                                        reader.onload = (event) => {
                                          if (event.target?.result) {
                                            const updated = { ...tempCharIntroState };
                                            updated[key].imageUrl = event.target.result as string;
                                            setTempCharIntroState(updated);
                                            setIsDirty(true);
                                          }
                                        };
                        ...
                        <div className="space-y-4 overflow-y-auto pr-2 flex-grow">
                          ...
                          {tempCharIntroState.others?.map((other: any, oIdx: number) => (
                            <div key={oIdx} className="p-3 bg-gray-50/50 rounded-xl border border-gray-100 space-y-3 relative">
                              ...
                              <div className="flex flex-col gap-1">
                                <label className="text-[10px] font-bold text-gray-500">頭像路徑 / 上傳新頭像：</label>
                                <div className="flex gap-2">
                                  <input 
                                    type="text"
                                    ...
                                    onChange={(e) => {
                                      const updated = { ...tempCharIntroState };
                                      updated.others[oIdx].avatarUrl = e.target.value;
                                      setTempCharIntroState(updated);
                                      setIsDirty(true);
                                    }}
                                    className="flex-grow px-3 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#8a7f9c] text-xs text-gray-700 font-mono"
                                    placeholder="https://..."
                                  />
                                  <label className="flex-shrink-0 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-200 rounded-lg text-xs font-semibold cursor-pointer transition-colors flex items-center justify-center">
                                    上傳
                                    <input 
                                      type="file"
                                      accept="image/*"
                                      ...
                                      onChange={(e) => {
                                        ...
                                        if (file) {
                                          ...
                                          reader.onload = (event) => {
                                            if (event.target?.result) {
                                              const updated = { ...tempCharIntroState };
                                              updated.others[oIdx].avatarUrl = event.target.result as string;
                                              setTempCharIntroState(updated);
                                              setIsDirty(true);
                                            }
                                          };
